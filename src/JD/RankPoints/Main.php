...
class Main extends PluginBase {
    ...
    public function onCommand(CommandSender $sender, Command $cmd, $label, array $args) {
        ...
        if (isset($args[0])) {$name = $args[0];}
        if (isset($args[1])) {$pointstogive = $args[1];}
        ...
        if ($sender instanceof Player) {
            ...
            if (!$this->playerRegistered($sender->getName()))  {$this->registerPlayer($sender->getName());}
        ...
        echo "Player is not online" . "\n";
            ...
            $newvotes = $data["votes"] + $pointstogive;
            ...
            if (!$currentRankIndex) return true;
